  ...
  return (
    <>
      ...
      <div className='w-10/12 mx-auto mt-20 flex flex-col-reverse md:flex-row justify-between items-center'>
        <p className='text-2xl font-bold text-[#131313]'>{btnToggle ? "Available Players" : "Selected Player (4/6)"}</p>
        <div className='border-2 border-gray-100 rounded-3xl mb-5'>
          ...
          <button onClick={() => setBtnToggle(false)} className={`py-1.5 md:py-3  px-3.5 md:px-7 rounded-r-3xl ${btnToggle === false ? 'bg-[#E7FE29] font-bold' : ''}`}>Selected <span>(0)</span></button>
        </div>
      </div>
      {
        btnToggle ? <Suspense fallback={<div className='w-10/12 mx-auto flex justify-center'><span className="loading loading-spinner loading-xl"></span></div>}>
          <Players purchasedPlayer={purchasedPlayer} playersLoad={playersLoad} balance = {balance} setBalance={setBalance}></Players>
